...
function resize() {
  ...
  if (canvas.width == 1366) {
    ...
    brick.column = Math.round(canvas.width / 200);
    brick.offSetLeft = 60;
    brick.marginTop = canvas.height / 4;
  }
  ...
  for (let r = 0; r < brick.row; r++) {
    for (let c = 0; c < brick.column; c++) {
      ...
      if (b.status) {
        ...
        context.fillStyle = "red";
        context.fillRect(b.x, (b.y += 10), brick.width, brick.height);
      }
    }
  }
}
...
function showGameStats(text, textX, textY, img, imgX, imgY) {
  ...
  context.font = "2.8rem Germania One";
...
function draw() {
  ...
  if (y - 60 + vely < ballRadius) {
    ...
    if (lives == 0) {
      ...
      customPop("Game Over. Your Score is : " + score);
      score = 0;
    } else {
      flag = 1;
      paddleX = (canvas.width - paddleWidth) / 2;
...
function showuser() {
  ...
  document.getElementById("initial").innerHTML = data[0].username
    .charAt(0)
    .toUpperCase();
  document.getElementById("initial1").innerHTML = data[0].username
    .charAt(0)
    .toUpperCase();
    ...
    for (let c = 0; c < brick.column; c++) {
      ...
      if (b.status == 1) {
        if (
          ...
          y - 60 + ballRadius > b.y &&
          y - 60 - ballRadius < b.y + brickHeight
        ) {
          vely = -vely;
